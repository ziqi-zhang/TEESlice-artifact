...
import knockoff.utils.model_feat as model_utils
...
def main():
    ...
    model_name = params['model_arch']
    # model = model_utils.get_net(model_name, n_output_classes=num_classes, pretrained=pretrained)
    model = zoo.get_net(model_name, modelfamily, params['pretrained'], num_classes=num_classes)

    model.conv1.load_state_dict(teacher.conv1.state_dict())
    model.bn1.load_state_dict(teacher.bn1.state_dict())
    model = model.cuda()
    ...
    for b in budgets:
        ...
        model_utils.train_model_feat(model, teacher, transferset, model_dir, testset=testset, criterion_train=criterion_train,
