...
class NetTailor(nn.Module):
    def __init__(self, backbone, num_classes, max_skip=1, pretrain_path=None):
        ...
        self.flops =[[float(cur_shape[1] * cur_shape[2]) * (3 * 64 * 3**2 + 64 * 2)]]
    ...
    def init_proxy_weights(self):
        for layer in self.layers:
            for m in layer.proxies.modules():
                if isinstance(m, nn.Conv2d):
                    nn.init.normal_(m.weight, mean=0, std=0.01)
                elif isinstance(m, nn.BatchNorm2d):
                    nn.init.constant_(m.weight, 1)
...
if __name__ == '__main__':
    ...
    print(model.stats())
    # full_model_dir = "checkpoints/svhn/nettailor-resnet18-3Skip-T10.0-C0.3"
    # import proj_utils
    # proj_utils.load_checkpoint(model, model_dir=full_model_dir)
    # model.to(device)
    # st()
