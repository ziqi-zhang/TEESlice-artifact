...
from knockoff.nettailor_face import teacher_resnet
from knockoff.nettailor_face import proj_utils
from knockoff.nettailor_face import dataloader as dataloaders
from knockoff.nettailor_face import dataset_info
...
parser.add_argument('--victim-arch', default='resnet18')
...
def main():
    ...
    if args.arch.startswith('resnet'):
        model = teacher_resnet.create_teacher(args.arch, pretrained=args.imagenet_pretrain, num_classes=num_classes, img_size=img_size)
    ...
    if mode == 'train':
        ...
        for epoch in range(args.epochs):
            ...
            train(train_loader, model, victim_model, criterion, optimizer, epoch, logger)
...
def train(data_loader, model, victim_model, criterion, optimizer, epoch, logger):
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        with torch.no_grad():
            victim_logit = victim_model(images)
            victim_label = victim_logit.max(1)[1]
        # compute output
        logit = model(images)
        # loss = criterion(logit, labels)
        loss = kdloss(logit, victim_logit.detach())
