...
def load_checkpoint(model, model_dir=None, model_fn=None, tensors=None, optimizer=None, outps=None, ignore_tensors=None):
    ...
    for k_ckp in checkpoint['state_dict']:
        ...
        if k_ckp.startswith('module.'):
            k_ckp_map = k_ckp[7:]
        else:
            k_ckp_map = k_ckp
        k_st = tensors[k_ckp] if tensors is not None else k_ckp_map
        if k_st in state:
