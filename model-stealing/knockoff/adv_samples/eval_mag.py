...
def main():
    ...
    for b in budgets:
        ...
        if b == 0:
            ...
            surrogate = weight_prune(surrogate, pretrained, 0.01)
            ...
            surrogate_attack_penultimate(surrogate, victim, f"budget{b}")
            surrogate_attack_output(surrogate, victim, f"budget{b}")
        else:
            ...
            surrogate = Blackbox.load_checkpoint(pretrained, adv_dir, f"checkpoint.{b}.None.pth.tar")
            surrogate = surrogate.cuda()
            surrogate.eval()
