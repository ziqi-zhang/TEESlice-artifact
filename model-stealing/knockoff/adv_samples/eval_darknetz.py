...
def main():
    ...
    parser.add_argument('--graybox_mode', type=str)
    ...
    def surrogate_attack_penultimate(surrogate, victim, prefix):
        ...
        with open(osp.join(params['out_path'], f"penult_{prefix}_result.txt"), 'w') as f:
            f.write(log)
        surrogate.attack_backbone = False
    ...
    for b in budgets:
        ...
        if b == 0:
            ...
            victim_dir = params['victim_dir']
            blackbox = Blackbox.from_modeldir(victim_dir, device)
            surrogate = blackbox._Blackbox__model
            ...
            if args.graybox_mode == "fc":
                ...
            elif args.graybox_mode == "lastconv":
                surrogate.layer4[-1].load_state_dict(pretrained.layer4[-1].state_dict())
            elif args.graybox_mode == "layer4":
                surrogate.layer4.load_state_dict(pretrained.layer4.state_dict())
            else:
            ...
            surrogate_attack_penultimate(surrogate, victim, f"{args.graybox_mode}_budget{b}")
            surrogate_attack_output(surrogate, victim, f"{args.graybox_mode}_budget{b}")
        else:
            ...
            surrogate = Blackbox.load_checkpoint(pretrained, adv_dir, f"checkpoint.{b}.{args.graybox_mode}.pth.tar")
            surrogate = surrogate.cuda()
            surrogate.eval()
