...
class SecretNNStudentResNetSmallFrevalds(nn.Module):
    ...
    def __init__(
        ...
        self.sgx_layers = [
            self.input_layer, self.conv1, self.bn1, self.conv1_freivalds_post,
            self.relu1, 
...
def secret_resnet50(pretrained=False, EnclaveMode=ExecutionModeOptions.Enclave, **kwargs):
    ...
    resnet_constructor = SecretNNStudentResNetSmallFrevalds(SecretBottleneckBlock, [3, 4, 6, 3], EnclaveMode=EnclaveMode, **kwargs)
    return resnet_constructor
...
def secret_resnet101(pretrained=False, EnclaveMode=ExecutionModeOptions.Enclave, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    resnet_constructor = SecretNNStudentResNetSmallFrevalds(SecretBottleneckBlock, [3, 4, 23, 3], EnclaveMode=EnclaveMode, **kwargs)
    return resnet_constructor

def eval_sgx_nettailor_time(
    arch="resnet18", root="nettailor/cifar10", pretrained_subdir="cifar100-resnet18",
    nettailor_subdir="nettailor", nettailor_full_subdir="full_model", batch_size=64,
    num_classes=10, num_repeat=1, max_skip=3
):
    pretrained = resnet_small.create_model(arch, num_classes=num_classes, max_skip=max_skip)
