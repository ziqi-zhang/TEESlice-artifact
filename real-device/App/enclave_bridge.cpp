 ...
 */
    ...
    void InitQuantTensor(EidT eid, IdT TenId, int dim0, int dim1, int dim2, int dim3) {
        DimsT Dims = CreateDims(dim0, dim1, dim2, dim3);
        // printf("InitQuantTensor id %llu \n", TenId);
        sgx_status_t ret = ecall_init_quant_tensor(eid, TenId, (void*) &Dims);
        if (ret != SGX_SUCCESS) { print_error_message(ret); throw ret; }
    }
    void SetQuantTen(EidT eid, IdT TenId, DtypeForCpuOp* Arr) {
        // printf("SetQuantTen id %llu \n", TenId);
        sgx_status_t ret = ecall_set_quant_ten(eid, TenId, (void*) Arr);
        if (ret != SGX_SUCCESS) { print_error_message(ret); throw ret; }
    }
    void GetQuantTen(EidT eid, IdT TenId, DtypeForCpuOp* Arr) {
        sgx_status_t ret = ecall_get_quant_ten(eid, TenId, (void*) Arr);
        if (ret != SGX_SUCCESS) { print_error_message(ret); throw ret; }
    }
...
//           std::cout << "Task is ready: " << task_future.get() << std::endl;
    ...
    void QuantReLUfunction(EidT eid, IdT TenIdin, IdT TenIdout, uint64_t size, int exp, int bits){
        sgx_status_t ret = ecall_quant_relu(eid, TenIdin, TenIdout, size, exp, bits);
        if (ret != SGX_SUCCESS) { print_error_message(ret); throw ret; }
    }
