...
class NetTailorBlock(nn.Module):
    ...
    def hetero_forward(self, ends):
        ...
        for i, (alpha, k) in enumerate(zip(self.alphas(), self.keep_flag)):
            ...
            if inp is None:
                continue
            out += alpha * (self.main.hetero_forward(inp.cuda()).cpu() if i==0 else self.proxies[i-1].hetero_forward(inp.cpu()))
        out.cpu()
        return F.relu(out, inplace=True) if not isinstance(out, float) else None
...
class HeteroNetTailor(nn.Module):
    ...
    def load_pretrained(self, pretrain_path):
        ...
        for k_st in state:
            ...
            if in_module:
                k_ckp = "module."+k_ckp
            print(k_ckp, checkpoint[k_ckp].size())
            print(k_st, state[k_st].size())
            if k_ckp == 'conv1.weight' and checkpoint[k_ckp].size()[2] == 3 and state[k_st].size()[2] == 7:
                checkpoint[k_ckp] = state[k_st]
...
def create_model(backbone, num_classes, max_skip, pretrain_path=None):
    return HeteroNetTailor(backbone, num_classes, max_skip=max_skip, pretrain_path=pretrain_path)
...
if __name__ == '__main__':
    ...
    full_model_dir = "nettailor_224/models_nettailor/models/CIFAR10-resnet18/resnet18-iterative-nettailor-3Skip-T10.0-C0.3-Pruned0"
    # import proj_utils
