...
import nettailor_224.student_resnet as student_resnet

from nettailor.utils import *
from nettailor_224.pytorch_resnet import *
...
def inject_res_block(block, pretrained_block):
    if isinstance(block, SecretBasicBlock):
        ...
        # block.conv1_freivalds_post.reset_plain_bn()
        # block.conv2_freivalds_post.reset_plain_bn()
        if len(block.downsample_layers) > 1:
            # assert isinstance(block.downsample_layers[0], SGXConvBase)
            # block.downsample_layers[0].inject_params(pretrained_block.conv_res)
            # assert isinstance(block.downsample_layers[1], SecretBatchNorm2dLayer)
            # block.downsample_layers[1].inject_params(pretrained_block.bn_res)
            # block.downsample_layers[2].reset_plain_bn()
...
class SecretBasicBlock():
    ...
    def __init__(self, inplanes, planes, sid, name_prefix, stride=1, downsample_layers=None, EnclaveMode=ExecutionModeOptions.Enclave):
        ...
        # self.conv1_freivalds_post = SecretBatchNorm2dLayer(
        #     sid, f"{name_prefix}.conv1_freivalds_post",EnclaveMode=ExecutionModeOptions.Enclave)

        # self.relu1 = SecretReLULayer(sid, f"{name_prefix}.relu1", EnclaveMode=ExecutionModeOptions.Enclave)
        self.relu1 = SecretEnclaveQuantReLULayer(
            sid, f"{name_prefix}.relu1", ExecutionModeOptions.Enclave
        )
        ...
        # explicitly implement Freivalds
        # self.bn2 = SecretBatchNorm2dLayer(
        #     sid=sid, LayerName=f"{name_prefix}.bn2", EnclaveMode=EnclaveMode, 
            
        # )
        # self.conv2_freivalds_post = SecretBatchNorm2dLayer(
        #     sid, f"{name_prefix}.conv2_freivalds_post",EnclaveMode=ExecutionModeOptions.Enclave,
        ...
        # )
        # self.add.register_prev_layer(self.conv2_freivalds_post)
        # self.conv2_freivalds_post.register_next_layer(self.add)
        # layers = [
        #     self.conv1, self.bn1, self.relu1, 
        #     self.conv2, self.bn2, self.conv2_freivalds_post, 
        # ]
...
class SecretBottleneckBlock():
    ...
    def __init__(self, inplanes, planes, sid, name_prefix, stride=1, downsample_layers=None, EnclaveMode=ExecutionModeOptions.Enclave):
        ...
        # self.relu2 = SecretReLULayer(sid, f"{name_prefix}.relu2", EnclaveMode=ExecutionModeOptions.Enclave)
        self.relu2 = SecretEnclaveQuantReLULayer(
            sid, f"{name_prefix}.relu2", ExecutionModeOptions.Enclave
        )
...
class SecretNettailorBlock():
    def __init__(
        ...
        if sum(self.keep_flag[1:]) > 0:
            ...
            # self.relu = SecretReLULayer(sid, f"{name_prefix}.relu", EnclaveMode=ExecutionModeOptions.Enclave)
            self.relu = SecretEnclaveQuantReLULayer(
                sid, f"{name_prefix}.relu", ExecutionModeOptions.Enclave
            )
...
class SecretNNStudentResNetSmallFrevalds(nn.Module):
    ...
    def __init__(
        ...
        super(SecretNNStudentResNetSmallFrevalds, self).__init__()
        ...
        header_enclave_mode = ExecutionModeOptions.GPU
        self.conv1 = SGXConvBase(
        ...
        # self.conv1_freivalds_post = SecretBatchNorm2dLayer(sid, f"conv1_freivalds_post",EnclaveMode=ExecutionModeOptions.Enclave)
        
        
        self.relu1 = SecretReLULayer(sid, f"relu1",header_enclave_mode)
        # self.relu1 = SecretEnclaveQuantReLULayer(sid, f"relu1",header_enclave_mode)
        self.maxpool1 = SecretMaxpool2dLayer(sid, f"maxpool1", filter_hw=3, stride=2, padding=1,EnclaveMode=header_enclave_mode)
        self.sgx_layers = [
            self.input_layer, self.conv1, self.bn1, 
            self.relu1, self.maxpool1
            # self.input_layer, self.conv1, self.bn1
        ]
        ...
        self.proxy_prior_layers.appendleft(self.maxpool1)
        ...
        self.layer1_blocks, self.layer1 = self._make_layer(
            block, 64, layers[0], sid=sid, prev_layer=self.maxpool1, name_prefix="Layer1", 
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            #     EnclaveMode=EnclaveMode, 
            # )
            # downsample_conv_freivalds_post = SecretBatchNorm2dLayer(
            #     sid, f"{name_prefix}.0.main.downsample.conv_freivalds_post",EnclaveMode=ExecutionModeOptions.Enclave,
            #     manually_register_next=True, link_next=False
            # )
            # downsample_layers = [downsample_conv, downsample_bn, downsample_conv_freivalds_post]
    ...
    def inject_params(self, pretrained):
        ...
        # self.conv1_freivalds_post.reset_plain_bn()
...
def secret_resnet18(pretrained_model, EnclaveMode=ExecutionModeOptions.Enclave, **kwargs):
    ...
    resnet_constructor = SecretNNStudentResNetSmallFrevalds(SecretBasicBlock, [2, 2, 2, 2], pretrained_model, EnclaveMode=EnclaveMode, **kwargs)
...
def secret_resnet34(pretrained=False, EnclaveMode=ExecutionModeOptions.Enclave, **kwargs):
    ...
    """
    # resnet_constructor = SecretNNResNet(BasicBlock, [2, 2, 2, 2], **kwargs)
    resnet_constructor = SecretNNStudentResNetSmallFrevalds(SecretBasicBlock, [3, 4, 6, 3], EnclaveMode=EnclaveMode, **kwargs)
    return resnet_constructor


def eval_sgx_nettailor_time(
    backbone="resnet18", root="nettailor_224/models_nettailor/models", pretrained_subdir="CIFAR10-resnet18",
    nettailor_subdir="nettailor", batch_size=64,
    num_classes=10, num_repeat=10, max_skip=3
):
    torch.set_num_threads(1)

    pretrained = student_resnet.create_model(backbone, num_classes=num_classes, max_skip=max_skip)
    ...
    if not osp.exists(pretrained_path):
        ...
        raise RuntimeError
    nettailor_dir = osp.join(root, nettailor_subdir)
    if not osp.exists(osp.join(nettailor_dir, "checkpoint.pth.tar")):
        print(f"nettailor_dir does not exist")
        print(f"nettailor_dir is {nettailor_dir}/checkpoint.pth.tar")
        raise RuntimeError

    pretrained.load_pretrained(pretrained_path)
    load_checkpoint(pretrained, model_dir=nettailor_dir)
    pretrained.eval()
    print(pretrained.alphas_and_complexities())
    GlobalTensor.init()
    resnet_constructor = eval(f"secret_{backbone}")(
    ...
    for i in range(num_repeat):
        ...
        inf_times.append(elapse_time / batch_size)
        throughputs.append(1000 / inf_times[-1])
    ...
    time_mean = np.mean(inf_times)
    time_std = np.std(inf_times)
    ...
    print(f"Time mean {time_mean:.2f}, std {time_std:.2f}. Throughputs mean {th_mean:.2f}, std {th_std:.2f}")
    return inf_times, throughputs


if __name__=="__main__":
    eval_sgx_nettailor_time()
    ...
    # pretrained = resnet_small.create_model("resnet18", num_classes=num_classes, max_skip=max_skip)
    # # pretrained = resnet_small.create_model("resnet50", num_classes=1000, max_skip=max_skip)
    # # pretrained = resnet34(pretrained=False)
    # # pretrained = resnet50(pretrained=False)
    # # pretrained = resnet101(pretrained=False)
    ...
    # root = "nettailor/cifar10"
    # pretrained_dir = osp.join(root, "cifar100-resnet18")
    # pretrained_path = osp.join(pretrained_dir, "checkpoint.pth.tar")
    # nettailor_dir = osp.join(root, "nettailor")
    # full_model_dir = osp.join(root, "full_model")
    ...
    # GlobalTensor.init()
    # resnet_constructor = secret_resnet18(
    #     EnclaveMode=ExecutionModeOptions.GPU, max_skip=max_skip, num_classes=num_classes,
    #     pretrained_model=pretrained, batch_size=batch_size
    # )
    # # resnet_constructor = secret_resnet50(EnclaveMode=ExecutionModeOptions.GPU, max_skip=max_skip)
