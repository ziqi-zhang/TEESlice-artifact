...
class NetTailor(nn.Module):
    def __init__(self, backbone, num_classes, max_skip=1, pretrain_path=None):
        ...
        self.flops =[[float(112 * 112) * (3 * 64 * 7**2 + 64 * 2)]]
    ...
    def load_pretrained(self, base_arch):
        ...
        # checkpoint['conv1.weight'] = checkpoint['conv1.weight'][:,:,2:-2,2:-2]
    ...
    def get_task_param_flop(self):
        ...
        num_global_params = sum(self.global_params())
        print(f"Global params: ", self.global_params())
        print(f"Total global params: ", num_global_params)
        num_task_param = sum([
            sum(params) for params in self.task_params()
        ])
        print(f"Task params: ", self.task_params())
        print(f"Total task params: ", num_task_param)

        global_flops = [f[0] for f in self.flops]
        num_global_flops = sum(global_flops)
        print(f"Global flops: ", global_flops)
        print(f"Total global flops: ", num_global_flops)

        num_task_flops = 0
        task_flops = []
        for l, ff in zip(self.layers,self.flops[1:-1]):
            layer_flop = [f if k else 0 for f, k in zip(ff, l.keep_flag)]
            task_flops.append(layer_flop[1:])
            num_task_flops += sum(layer_flop[1:])
        print(f"Task flops: ", task_flops)
        print(f"Total Task flops: ", num_task_flops)

        # print(self.stats())
        return num_global_params, num_task_param, num_global_flops, num_task_flops
...
def summarize_student_model(dataset_name="CIFAR10", model_name="resnet18"):
    ...
    path = osp.join(
        f"nettailor_224/models_nettailor/models/{dataset_name}-{model_name}/resnet18-iterative-nettailor-3Skip-T10.0-C0.3-Pruned0",
    )
    assert os.path.exists(path)

    load_checkpoint(model, model_dir=path)
    num_global_params, num_task_param, num_global_flops, num_task_flops = model.get_task_param_flop()
    print(f"Global param {num_global_params}, task param {num_task_param}, global flops {num_global_flops}, task flops {num_task_flops}")
    return num_global_params, num_task_param, num_global_flops, num_task_flops
...
if __name__ == '__main__':
    ...
    #     for dataset_name in ["CIFAR10"]:
            ...
            num_global_params, num_task_param, num_global_flops, num_task_flops = summarize_student_model(dataset_name, model_name)
            result["global_params"] = round(num_global_params/10**6, 2)
            result["task_params"] = round(num_task_param/10**6, 2)
            result["global_flops"] = round(num_global_flops/10**9, 2)
            result["task_flops"] = round(num_task_flops/10**9, 2)
            results.append(result)
    
    path = osp.join(f"nettailor_224/models_nettailor", "param_flop_result.json")
