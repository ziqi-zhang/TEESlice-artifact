...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=1000, img_size=224, num_fc_features=512, pretrained=False):
        super(VGG, self).__init__()
        self.features = features
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.classifier = nn.Linear(num_fc_features, num_classes)
...
def pruned_vgg16_bn(pretrained=False, ratio=0.5, **kwargs):
    """VGG 16-layer model (configuration "D") with batch normalization"""
    model = VGG(
        pruned_make_layers(cfg['D'], ratio, batch_norm=True), 
        num_fc_features=int(512*ratio),
        **kwargs
    )

    return model

def pruned_vgg19_bn(pretrained=False, ratio=0.5, **kwargs):
    """VGG 16-layer model (configuration "D") with batch normalization"""
    model = VGG(
        pruned_make_layers(cfg['E'], ratio, batch_norm=True), 
        num_fc_features=int(512*ratio),
        **kwargs
    )
...
def test_vgg16(batch_size, iteration):
    mean_times, mean_throughputs = [], []
    error_times, error_throughputs = [], []
    params, flops = [], []
...
def test_vgg19(batch_size, iteration):
    mean_times, mean_throughputs = [], []
    error_times, error_throughputs = [], []
    params, flops = [], []
    ...
    for ratio in [0.05, 0.1, 0.3, 0.5, 0.7, 0.8, 0.9, 0.95, 1.0]:
        ...
        model = pruned_vgg19_bn(num_classes=10, ratio=ratio)
    ...
    print(f"vgg19_bn_prune_params = ", params)
    print(f"vgg19_bn_prune_flops = ", flops)

    print(f"vgg19_bn_prune_time_mean = ", mean_times)
    print(f"vgg19_bn_prune_time_error = ", error_times)
    print(f"vgg19_bn_prune_throughput_mean = ", mean_throughputs)
    print(f"vgg19_bn_prune_throughput_error = ", error_throughputs)
...
if __name__=="__main__":
    ...
    test_vgg19(batch_size, iteration)