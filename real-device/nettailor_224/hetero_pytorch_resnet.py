...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, img_size=224, attack_backbone=False):
        self.inplanes = 64
...
def hetero_time(model, input, iteration, batch_size):
    model.hetero_deploy()
    input = input.cuda()
...
def unified_time(model, input, iteration, batch_size):
    model(input)
...
if __name__=="__main__":
    ...
    model = resnet18(num_classes=10, img_size=224)
    if cuda:
        model = model.cuda()
    batch_size = 64
    iteration = 10
    ...
    # hetero_time(model, input, iteration, batch_size)
    unified_time(model, input, iteration, batch_size)