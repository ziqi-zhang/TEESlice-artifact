...
class GlobalTensor(object):
    ...
    @staticmethod
    def remove_gpu(tag):
        GlobalTensor.gpu_tensors.pop(tag)
    ...
    @staticmethod
    def init_enclave_quant_tensor(tag, size):
        size = list(size)
        if len(size) < 4:
            size = [1] * (4 - len(size)) + size
        remapped_tag = GlobalTensor.get_remapped_tags(tag)
        if remapped_tag in GlobalTensor.IsInitEnclaveTensor:
            ...
            GlobalTensor.IsInitEnclaveTensor[remapped_tag] = True
        eid = GlobalTensor.get_eid()
        # print("init_enclave_quant_tensor: tag ", tag, " remapped_tag ", remapped_tag)
        GlobalTensor.EnclaveInterface.lib.InitQuantTensor(eid, remapped_tag, size[0], size[1], size[2], size[3])
        # print("init_enclave_quant_tensor: ", GlobalTensor.cpu_tensor.keys())
...
class EnclaveInterface(object):
    ...
    def __init__(self):
        ...
        self.lib.InitQuantTensor.argtypes = [self.EidT, self.IdT] + [c_int] * 4
        self.lib.SetQuantTen.argtypes = [self.EidT, self.IdT, self.ArrEncryptedT]
        self.lib.GetQuantTen.argtypes = [self.EidT, self.IdT, self.ArrEncryptedT]
        ...
        self.lib.QuantReLUfunction.argtypes = [self.EidT, self.IdT, self.IdT, c_uint64, c_int, c_int]
    ...
    def init_enclave_quant_tensor(self, name, size):
        GlobalTensor.init_enclave_quant_tensor(self.get_tag(name), size)
    ...
    def set_quant_tensor(self, name, tensor):
        self.lib.SetQuantTen(self.get_eid(), self.get_tag(name), get_encryption_ptr(tensor))

    def get_tensor(self, name, tensor):
        self.lib.GetTen(self.get_eid(), self.get_tag(name), get_float_ptr(tensor))

    def get_quant_tensor(self, name, tensor):
        self.lib.GetQuantTen(self.get_eid(), self.get_tag(name), get_encryption_ptr(tensor))
    ...
    def quant_relunew(self, namein, nameout, sizelist, exp, bits):
        self.lib.QuantReLUfunction(self.get_eid(), self.get_tag(namein), self.get_tag(nameout), np.prod(sizelist), exp, bits)
    ...
    # Reuse of tensor during one execution of DNN
    tensor_reuse_info = {}
    @staticmethod
    def init_tensor_reuse_info():
        EnclaveInterface.tensor_reuse_info = {}
        print("="*30, "Init Tensor Reuse Info", "="*30)
        for tag, friends in GlobalTensor.InverseLinkedTags.items():
            reuse_list = set()
            for f in friends:
                reuse_list.add(EnclaveInterface.tag_to_names[f])
            EnclaveInterface.tensor_reuse_info[EnclaveInterface.tag_to_names[tag]] = reuse_list

    @staticmethod
    def print_gpu_tensors_info():
        info = ""
        for tag in GlobalTensor.gpu_tensors.keys():
            info += f"{EnclaveInterface.tag_to_names[tag]}, "
        print(info)
