...
class SGXConvBase(SecretLayerBase):
    ...
    def forward(self):
        with NamedTimerInstance(f"S{self.sid}: {self.LayerName} ConvLayerUnitForward", verbose_level=VerboseLevel.LAYER):
            self.forward_tensor_transfer("input")
            if self.EnclaveMode == ExecutionModeOptions.Enclave:
                with NamedTimerInstance(f"S{self.sid}: {self.LayerName} ConvLayerUnitEnvlaveForward", verbose_level=VerboseLevel.LAYER):
                    # "input" is pytorch form
                    with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} ConvLayerEnclaveInputPreprocess", verbose_level=VerboseLevel.LAYER):
                    ...
                    with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} ConvLayerEnclaveForward", verbose_level=VerboseLevel.LAYER):
                        # if self.LayerName == "Layer2.0.downsample.conv":
                        #     st()
                        self.sgx_conv_forward(self.LayerName)
                    
                    with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} ConvLayerEnclaveOutputPreprocess", verbose_level=VerboseLevel.LAYER):
                ...
                with NamedTimerInstance(f"S{self.sid}: {self.LayerName} ConvLayerUnitCPUForward", verbose_level=VerboseLevel.LAYER):
                    with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} ConvLayerCPUInputPreprocess", verbose_level=VerboseLevel.LAYER):
                        self.forward_tensor_transfer()

                    # with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} get CPU input", verbose_level=VerboseLevel.LAYER):
                    #     input = self.get_cpu("input")
                    # with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} get CPU forward", verbose_level=VerboseLevel.LAYER):
                    #     output =  self.ForwardFunc(input)
                    # with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} set CPU output", verbose_level=VerboseLevel.LAYER):
                    #     self.set_cpu("output", output)
                    with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} ConvLayerCPUForward", verbose_level=VerboseLevel.LAYER):
                        self.set_cpu("output", self.ForwardFunc(self.get_cpu("input")))
            elif self.EnclaveMode == ExecutionModeOptions.GPU:
                with NamedTimerInstance(f"S{self.sid}: {self.LayerName} ConvLayerUnitGPUForward", verbose_level=VerboseLevel.LAYER):
                    with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} ConvLayerGPUInputPreprocess", verbose_level=VerboseLevel.LAYER):
                        self.forward_tensor_transfer()

                    with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} ConvLayerGPUForward", verbose_level=VerboseLevel.LAYER):
                        # self.set_gpu("output", self.ForwardFunc(self.get_gpu("input").type(SecretConfig.dtypeForCpuOp)))
                        with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} GPU get input", verbose_level=VerboseLevel.LAYER):
                            input = self.get_gpu("input").type(SecretConfig.dtypeForCpuOp)
                        with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} GPU compute", verbose_level=VerboseLevel.LAYER):
                            output = self.ForwardFunc(input)
                        with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} GPU set output", verbose_level=VerboseLevel.LAYER):
                            self.set_gpu("output", output)
            self.check_free_gpu_tensor("input")
