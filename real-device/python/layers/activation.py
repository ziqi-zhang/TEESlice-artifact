...
class SecretActivationLayer(SecretNonlinearLayer):
    ...
    def forward(self):
        with NamedTimerInstance(f"S{self.sid}: {self.LayerName} ActivationLayerUnitForward", verbose_level=VerboseLevel.LAYER):
            with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} ActivationLayerUnitInputPreprocess", verbose_level=VerboseLevel.LAYER):
            ...
            if self.EnclaveMode == ExecutionModeOptions.Enclave:
                ...
                with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} ActivationLayerEnclaveForward", verbose_level=VerboseLevel.LAYER):
                ...
                if self.PrevLayer.EnclaveMode is not ExecutionModeOptions.CPU and torch.sum(self.get_cpu("input").abs()) == 0:
                    raise RuntimeError(f"{self.LayerName}: SGX input not load")
                with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} ActivationLayerCPUForward", verbose_level=VerboseLevel.LAYER):
                ...
                with NamedTimerInstance(f"  S{self.sid}: {self.LayerName} ActivationLayerGPUForward", verbose_level=VerboseLevel.LAYER):
