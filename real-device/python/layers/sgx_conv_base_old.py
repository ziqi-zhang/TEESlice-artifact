...
class SGXConvBase(SecretLayerBase):
    ...
    def __init__(
        ...
        super().__init__(sid, LayerName, link_prev, link_next, manually_register_prev, manually_register_next)
        ...
        with NamedTimerInstance(f"S{self.sid}: {self.LayerName} Forward", verbose_level=VerboseLevel.LAYER):
            if self.is_enclave_mode:
                ...
                with NamedTimerInstance(f"S{self.sid}: {self.LayerName} Input Preprocess", verbose_level=VerboseLevel.LAYER):
                    if self.PrevLayer is not None and self.PrevLayer.StoreInEnclave is True:
    ...
    def print_connection_info(self):
        print(f"{self.LayerName:30} shape{self.pytorch_x_shape}{' ':30} input {self.PrevLayer.LayerName:30} output {self.NextLayer.LayerName:30}")
