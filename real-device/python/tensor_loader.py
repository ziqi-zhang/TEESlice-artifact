...
class TensorLoader(EnclaveInterface):
    ...
    def remove_gpu(self, name):
        GlobalTensor.remove_gpu(self.get_tag(name))
    ...
    def generate_cpu_quant_tensor(self, name, shape):
        self.set_cpu(name, torch.zeros(shape).type(SecretConfig.dtypeForQuantCpuOp))
    ...
    # ********** Quant **********
    def quant_transfer_cpu_to_gpu(self, name):
        assert self.get_cpu(name).dtype is torch.uint8
        self.set_gpu(name, self.get_cpu(name).cuda(non_blocking=True).type(SecretConfig.dtypeForQuantCudaMm))
        # self.GpuTensors[name] = self.CpuTensors[name].cuda(non_blocking=True).type(SecretConfig.dtypeForCudaMm)

    def quant_transfer_gpu_to_cpu(self, name):
        cpu_tensor = self.get_cpu(name)
        gpu_tensor = self.get_gpu(name)
        cpu_tensor.copy_(gpu_tensor.type(SecretConfig.dtypeForQuantCpuOp))

    def quant_transfer_enclave_to_cpu(self, name):
        # self.from_enclave(name, self.get_cpu(name))
        self.get_quant_tensor(name, self.get_cpu(name))

    def quant_transfer_cpu_to_enclave(self, name):
        self.set_quant_tensor(name, self.get_cpu(name))
    # ********** End of Quant **********
