...
#             total_bn += module.weight.data.numel()
...
#     bn_weights = torch.zeros(total_bn)
#     index = 0
#     for name, module in pretrained.named_modules():
#         if ( isinstance(module, nn.BatchNorm2d) ):
#             size = module.weight.data.numel()
#             bn_weights[index:(index+size)] = module.weight.data.view(-1).abs().clone()
#             index += size
    
#     y, i = torch.sort(bn_weights, descending=True)
#     bn_thre_index = int(total_bn * prune_ratio)
#     if prune_ratio == 1:
#         bn_thre = y[-1]
#     else:
#         bn_thre = y[bn_thre_index]
#     log = f"Pruning bn threshold: {bn_thre:.4f}"
#     print(log)
...
#         elif (isinstance(module, nn.Linear)):
#             module.weight.data.copy_(p_module.weight.data)
#             module.bias.data.copy_(p_module.bias.data)

#         elif (isinstance(module, nn.BatchNorm2d)):
#             weight_copy = p_module.weight.data.abs().clone()
#             mask = weight_copy.lt(bn_thre).float()
#             inv_mask = weight_copy.gt(bn_thre).float()

#             pruned_bn = pruned_bn + mask.numel() - torch.sum(mask)
#             # np.random.shuffle(mask)
#             module.weight.data.mul_(mask)
...
#     log = (f"Total bn params: {total_bn}, Pruned bn params: {pruned_bn}, "
#     f"Pruned ratio: {pruned_bn/total_bn:.2f}")
#     print(log)
